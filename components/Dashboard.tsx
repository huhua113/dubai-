...
import { Landmark, TrendingDown, TrendingUp, Wallet, SlidersHorizontal, LayoutDashboard, Repeat, BedDouble, Ruler, Calculator, FileDown } from 'lucide-react';

type MainView = 'dashboard' | 'upfront' | 'recurring';
type MobileOverlay = 'none' | 'settings';
type ChartRange = 'monthly' | 'yearly' | '5y' | '10y' | '25y';

const createNewProperty = (name: string): Property => {
    return {
        id: `prop_${Date.now()}`,
        name: name,
        settings: {
...
const Dashboard: React.FC = () => {
  ...
  const handleAddProperty = useCallback(() => {
    ...
    const newProperty = createNewProperty(newPropertyName);
  ...
  return (
    <div className="min-h-screen flex flex-col md:flex-row font-sans text-slate-800 bg-brand-bg">
      ...
      {mobileOverlay === 'settings' && activeProperty && (
